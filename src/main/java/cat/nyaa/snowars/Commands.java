    ...
    public static class ProducerCommand extends CommandReceiver{
        ...
        public void onSummon(CommandSender sender, Arguments arguments) {
            if (sender instanceof Player) {
                ...
                Team team = SnowarsPlugin.plugin.getServer().getScoreboardManager().getMainScoreboard().getTeam(s);
                ProducerConfig producerConfig = ProducerManager.getInstance().getConfig(producerConfigName);
                if (team == null){
                    new Message(I18n.format("producer.summon.error_no_team", s)).send(sender);
                    return;
                }
                if (producerConfig == null){
                    new Message(I18n.format("producer.summon.error_no_config", s)).send(sender);
                    return;
                }
                Block targetBlock = ((Player) sender).getTargetBlock(null, 10);
                Location spawnLoc = targetBlock.getLocation().add(0.5, 1, 0.5);
                ProducerManager.getInstance().summonProducer(spawnLoc, team, producerConfig);
                new Message(I18n.format("summon.success", s)).send(sender);
                ProducerManager.getInstance().save();
            }
        }
        ...
        public List<String> summonCompleter(CommandSender sender, Arguments arguments){
            ...
            switch (arguments.remains()){
                case 1:
                    Set<Team> teams = SnowarsPlugin.plugin.getServer().getScoreboardManager().getMainScoreboard().getTeams();
                    teams.forEach(team -> s.add(team.getName()));
                    break;
                case 2:
